...
import { Form, Input, InputNumber, Button, Anchor, Modal, Flex, message } from 'antd';
 ...
 * (state 管理 + Form 布局 + deployContract + Modal 展示合约地址)
 *
 * props:
 *   abiKey       cfg.js 里的合约 key: 'erc20' | 'divident' | 'flashMint' | 'BaseDao'
 *   fields       Form.Item 字段定义: [{ name, label, type, defaultValue, addonAfter, rules, group }]
 *   buildArgs(values, accountAddress) -> any[]  构造部署 args
 *   submitText   创建按钮文案
 *   successText  创建成功按钮文案 (可选)
 */
function DeployForm({ abiKey, fields, buildArgs, submitText = '创建合约代币' }) {
  const account = getAccount(config);

  // 用单 object state 替代 N 个 useState,字段动态生成
  ...
  // 部署状态机: idle | deploying | deployed | error
  ...
  // 字段按 group 分组渲染 (用于 Erc20claim 那种 "买入税率/卖出税率" 分组)
  ...
  const onPollAddress = async () => {
    ...
    try {
      ...
      setContractAddress(receipt.contractAddress || '(非合约创建交易)');
      setError('');
    } catch (e) {
      const errMsg = e?.shortMessage || e?.message || String(e);
  ...
  return (
    <Form layout="vertical">
      {groupedFields.map((g, gi) => (
        <div key={gi}>
          {g.title && <Anchor style={{ marginTop: 8 }}>{g.title}</Anchor>}
